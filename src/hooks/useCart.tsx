...
export function CartProvider({ children }: CartProviderProps) {
  const [cart, setCart] = useState<Cart[]>(() => {
    const storageCart = localStorage.getItem('@ZehCoffeeDelivery:cart')
  ...
  const totalShipping = totalProducts + 3.5
  ...
  function addCart(product: Cart) {
    ...
    setCart(copyCart)
    localStorage.setItem('@ZehCoffeeDelivery:cart', JSON.stringify(copyCart))
  }
